...
import { City, StateData, CityData } from "../types";
    ...
    const loadStateData = async () => {
      if (selectedCountry) {
        ...
        if (stateList.length === 0) {
          const directCities =
            CSCCity.getCitiesOfCountry(selectedCountry) || [];
  ...
  useEffect(() => {
    const loadCityData = async () => {
      if (selectedState && selectedCountry) {
        const cityList = CSCCity.getCitiesOfState(
          selectedCountry,
          selectedState,
        );
        setCities(cityList);
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row gap-4">
      ...
      <div className="flex-1 w-full">
        ...
        >
          <option value="">
            {cities.length === 0 && (selectedState || selectedCountry)
              ? "No Cities Found"
              : "Select a City"}
          </option>
          {cities.map((c, i) => (
