...
export function ChartsSection({
  ...
  for (let i = 0; i <= years; i++) {
    ...
    const projectedCo2Reduction =
      results.co2_reduction * progress * futureMultiplier;
    const co2 = Math.max(0, baseCo2 * (1 - projectedCo2Reduction / 100));

    const projectedAqiImprovement =
      results.aqi_improvement * progress * futureMultiplier;
    ...
    <div className="bg-slate-900/50 border border-slate-700/50 rounded-2xl p-6 backdrop-blur-sm shadow-xl h-80 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-slate-100 font-semibold flex items-center gap-2 relative z-10">
          ...
          {futureMode && (
            <span className="text-cyan-400 text-xs px-2 py-0.5 border border-cyan-500/30 bg-cyan-500/10 rounded-full">
              2035 Mode Active
            </span>
          )}
        </h3>
